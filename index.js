...
(async () => {
    ...
    page.on("response", async (response) => {
        ...
        // 👉 只處理 591 房屋列表 API
        if (!url.includes("bff-house.591.com.tw/v1/web/sale/list"))
            return;
        try {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        // =====================================================
        await page.waitForResponse(res => res.url().includes("sale/list"));
        // =====================================================
        ...
        await page.evaluate((mapObj) => {
            // 👉 把 Map 轉回 JS object
            const map = new Map(Object.entries(mapObj).map(([k, v]) => [Number(k), v]));
            // 👉 找所有房屋卡片
            ...
            rows.forEach((row) => {
                ...
                const title = row.querySelector('.ware-item__header') ||
                    row.querySelector('[title]');
                // 👉 如果有資料 → 上色
                if (title && color) {
                    title.style.setProperty("color", color, "important" // 🔥 強制覆蓋 CSS
                    );
                }
            });
